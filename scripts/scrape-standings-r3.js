...
const SOURCE_URL =
  "https://ffbs.wbsc.org/fr/events/2026-auvergne-rhone-alpes-championnat-r3-baseball-2026/standings";

const OUTPUT_PATH = path.join(process.cwd(), "data", "standings-r3.json");
...
function extractStandingsTable(html) {
  ...
  if (!bestTable) {
    throw new Error(
      "Aucun tableau trouvé sur la page. La structure du site a peut-être changé — vérifie le fichier de debug (debug/page-r3.html) pour diagnostiquer."
    );
  }
...
async function saveDebugFiles(html) {
  ...
  await writeFile("debug/page-r3.html", html, "utf-8");
}
